...
public class GeneralUtil {
	...
	public static boolean judgeZipcode(String number) {
		return number.matches("^[1-9][0-9]{5}$");
	}
	 ...
	 * 打开拨号盘，并将电话号码显示到拨号盘
	 */
	public static void showAnyPhoneDial(Context context, String phoneNumber) {
		// 先判断电话号码对不对
		if (judgeTelephone(phoneNumber) || judgePhoneQual(phoneNumber)) {
			// 打开拨号盘，并将电话号码显示到拨号盘
			Intent intent = new Intent();
			intent.setAction(Intent.ACTION_VIEW);
			intent.setData(Uri.parse("tel:" + phoneNumber));
	 ...
	 * 直接打电话
	 */
	public static void callPhoneNotCheck(Context context, String phoneNumber) {
		// 打开拨号盘，并将电话号码显示到拨号盘
		Intent intent = new Intent();
		intent.setAction(Intent.ACTION_VIEW);
		intent.setData(Uri.parse("tel:" + phoneNumber));
