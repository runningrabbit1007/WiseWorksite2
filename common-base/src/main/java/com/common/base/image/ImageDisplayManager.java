...
public class ImageDisplayManager extends ImageDisplayBase {
    ...
    public void display(Object with, ImageView imageView, Object resourceType, FileType fileType, int placeHolder, int errorResource, int roundRadius) {
        ...
        if (resourceType instanceof String) {
            request = requestManager.load((String) resourceType);
        ...
        if (fileType == FileType.CIRCLE) {
            //加载圆形图片转换器
            requestBuilder = request.transform(new GlideCircleTransform(defContext));
            requestBuilder.placeholder(placeHolder == -1 ? -1 : placeHolder)
                    .error(errorResource == -1 ? -1 : errorResource).crossFade().diskCacheStrategy(DiskCacheStrategy.ALL).into(imageView);
        } else if (fileType == FileType.NORMAL) {
            request.placeholder(placeHolder == -1 ? -1 : placeHolder)
                    .error(errorResource == -1 ? -1 : errorResource).crossFade().diskCacheStrategy(DiskCacheStrategy.ALL).into(imageView);
        } else if (fileType == FileType.ROUND) {
            if (roundRadius == -1) {
                roundRadius = 7;
            }
            requestBuilder = request.transform(new GlideRoundTransform(defContext, roundRadius));
            requestBuilder.placeholder(placeHolder == -1 ? -1 : placeHolder)
                    .error(errorResource == -1 ? -1 : errorResource).crossFade().diskCacheStrategy(DiskCacheStrategy.ALL).into(imageView);
        } else if (fileType == FileType.KEEP_SCALE) {
            requestBuilder = request.transform(new GlideKeepScaleTransform(defContext));
            requestBuilder.placeholder(placeHolder == -1 ? -1 : placeHolder)
                    .error(errorResource == -1 ? -1 : errorResource).crossFade().diskCacheStrategy(DiskCacheStrategy.ALL).into(imageView);
        }
    }
