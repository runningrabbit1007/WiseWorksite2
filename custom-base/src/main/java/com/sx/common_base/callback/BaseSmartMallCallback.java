...
import com.common.utils.URL.URL;
...
import com.sx.common_base.result.BaseMallResult;
...
/**
 * Created by chenxy on 2016/8/2.
 * Smartmall Project-商城部分 的基础回掉
 * DESC:
 * 20171113:修改回掉方法，空数据不会调用onSuccess方法，在onSuccess的实现中不需要在判断空。相应的如果数据为空，则会调用onGetFailed code 为 0
 */
public abstract class BaseSmartMallCallback extends Callback<BaseMallResult> {
    ...
    public BaseSmartMallCallback(Context mContext) {
        try {
    ...
    public BaseSmartMallCallback(Context mContext, boolean isFinish) {
        try {
            this.mContext = (Activity) mContext;
    ...
    @Override
    public void onResponse(BaseMallResult response, int id) {
         if(null == response){
            return;
        }
        ...
        if (response.getCode() == 200) {
            ...
            }else if (response.getCode() == ErrorCode.E100) {
                ToastUtil.showToast(App.INSTANCE, response.getMsg());
            }
            if (response.getCode() == ErrorCode.E500){
                ToastUtil.showToast(App.INSTANCE, response.getMsg());
            }
        }
        hindDialog();
    ...
    @Override
    public BaseMallResult parseNetworkResponse(Response response, int id) throws Exception {
        BaseMallResult result = null;
        String body = response.body().string();

        try {
            result = JSONUtil.parseObject(body, BaseMallResult.class);
        } catch (Exception e) {
            e.printStackTrace();
            result = null;
        }
    ...
    private void doSave(Response response, BaseMallResult result){
