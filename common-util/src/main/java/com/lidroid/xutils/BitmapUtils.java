...
public class BitmapUtils implements TaskHandler {
	...
	public <T extends View> void display(T container, String uri,
		...
		if (TextUtils.isEmpty(uri)) {
			ImageUtil.getInstance(context).display(container,
					ImageUtil.getImageOption().getErrorSourceId() + "");
			return;
		}
		...
		protected void onPostExecute(Bitmap bitmap) {
			...
			if (container != null) {
				if (bitmap != null) {
					...
					if(!ImageUtil.getImageOption().errorSourceIdIsZero()){
						ImageUtil.getInstance(context).display(container,
								ImageUtil.getImageOption().getErrorSourceId() + "");
					}
				}
	...
	public void clearAllCache(String uri){
		globalConfig.getBitmapCache().clearCache(uri);
	}
}
